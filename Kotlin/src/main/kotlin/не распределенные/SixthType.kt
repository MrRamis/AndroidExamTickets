...
//Разработайте программу, осуществляющую нахождение цифр, которые входят в каждое из данных натуральных чисел.
    fun task1(){
        readlnOrNull()?.let { string -> if (string.isNotEmpty() && string.isNotBlank()) string.split(" ").let { values -> values.flatMap { it.toSet() }.groupBy { it }.filterValues { it.size == values.size }.keys.let { println(it) } } else println("Error: String is empty") } ?: println("Error: String is null")
    }
